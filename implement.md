...
**Priority**: CRITICAL
**Why**: Random Urban Dictionary entries ≠ Gen Z slang. Your value prop breaks without this.
...
├── components/
│   ├── TermCard.tsx              # Main term display
│   ├── LanguageToggle.tsx        # ES/FR switch
│   ├── Waveform.tsx              # Audio visualization
│   ├── BrutalButton.tsx          # Neo-brutalist button
│   └── SearchInput.tsx           # Search field
├── hooks/
│   ├── useTheme.ts               # Theme management
│   └── useTermSearch.ts          # Fuse.js search
├── store/
...
├── data/
│   └── terms.json                # Bundled slang data (copy from preprocessing)
└── types/
...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      prevTerm: () => set((state) => ({
        currentIndex: (state.currentIndex - 1 + state.terms.length) % state.terms.length,
      })),
    ...
    <View style={[styles.card, { 
      ...
      {term.example && (
        <View style={[styles.exampleBox, { 
          ...
          {translation.example && (
            <>
              <View style={[styles.exampleDivider, { borderColor: theme.border }]} />
      ...
      <Text style={[styles.counter, { color: theme.textMuted }]}>
        {String(currentIndex + 1).padStart(2, '0')}/{String(terms.length).padStart(2, '0')}
      </Text>
    </View>
  );
}
...
export default function BrowseScreen() {
  ...
  const fuse = useMemo(() => new Fuse(terms, {
    keys: ['term'],
    threshold: 0.3,
  }), [terms]);
